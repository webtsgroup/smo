...
class library
{
    ...
    {        
		...
		$str3 = preg_replace('/\s\s+/',' ', trim($str3));
		$str3=preg_replace('/[^a-zA-Z0-9]/','-',$str3);
        return strtolower($str3);
    }   
    function getIdFoot($str) 
	{
		$str = preg_replace('#.html$#','',$str);
		if(!preg_match('/[_A-Za-z0-9-]-([0-9]+)$/i',$str,$match))
		return false;
		return $match[1];
	}
    function getidhead($str)
	{
		$tempArray=split('-',$str);
        return $tempArray[0];
	}
	function move_file($cur,$new)
	{
    ...
    function upload($config){
		...
		$default = array(
			'file' => 'file',
			'allowType' => array('ipeg','jpg','png','gif'),
			'allowSize' => 10485760, 
			'remove' => false,
			'createThumb' => true,
			'thumbSize' => "w=200&h=150&zc=1"
		);
		$UPLOAD = array_merge($default,$config);
		
		$remove = $UPLOAD['remove'];
		$field = $UPLOAD['file'];
		$img='';
		$ROOT = WWW_ROOT.'uploads/';
		$folder = $ROOT.$UPLOAD['folder'];
		$result = array(
			'success' => false,
			'message' => '',
			'img' => ''
		...
		$error = false;
		if($_FILES[$field]['name'] || $_POST[$field]) {
			if($_FILES[$field]['name'] && !$this->img_type($_FILES[$field]['name'],$UPLOAD['allowType']))
			{			
				$result['msg'] = 'Định dạng file không cho phép';
				return $result;
			}
			if($_FILES[$field]['size'] > $UPLOAD['allowSize'])
			{
				$result['msg'] = 'Kích thướt file quá lớn';
				return $result;
			}
			$img =$this-> make_upload_url($_FILES[$field]['name']);
			if(move_uploaded_file ($_FILES[$field]['tmp_name'],$folder."/".$img)) {
				//$thumb->GenerateThumbFile($folder."/".$img,$folder."/thumb/".$img);
				$result['img'] = $img;
				$result['success'] = true;
				if($UPLOAD['createThumb'])
				{
					//resize + create thumb
					$this->img_resize($folder."/".$img,$folder."/thumb/".$img,$UPLOAD['thumbSize']);
				}
				if($remove){
					$img_remove = $folder."/".$remove;
					$thumb_remove = $folder."/thumb/".$remove;
					if(file_exists($img_remove)&&is_file($img_remove))
					unlink($img_remove);
					if(file_exists($thumb_remove)&&is_file($img_remove))
					unlink($thumb_remove);
				}
			}
			else {
    ...
    function remove($file,$folder){
    	$img_remove = $folder."/".$file;
    	$thumb_remove = $folder."/thumb/".$file;
    	if(file_exists($img_remove)&&is_file($img_remove))
    	unlink($img_remove);
    	if(file_exists($thumb_remove)&&is_file($thumb_remove))
    	unlink($thumb_remove);
    }   
    function img_type(&$url,$img_arr = array()) {
		...
		{
			if (strstr($url,'.php') || strstr($url,'.asp') || strstr($url,'.jsp') || strstr($url,'.pl') || strstr($url,'.cgi')) 
				return false;
			else
				return true;
		}
    ...
    function make_upload_url($file) {
    	$t_url = strtolower($file);
    	$ext = explode('.',$t_url);
    	$ext = $ext[count($ext)-1];
    	$ext = explode('?',$ext);
    	$ext = $ext[0];
    	$file2 = $this->seoURL($file).'-'.NOW;
        return $file2.'.'.$ext;
    } 
	function get_file_name_extensions($file) {
    	$t_url = strtolower($file);
    	$ext = explode('.',$t_url);
    	$ext = $ext[count($ext)-1];
    	$ext = explode('?',$ext);
    ...
    {
        if (stripos($email,' ')) return false;
        elseif (!stripos($email,'@')) return false;
        elseif (stripos($email,'@')>strripos($email,'.')) return false;
        else return true;        
    }   
    function toCurrency($money)
	{
		...
		for ($i=0;$i<count($a);$i++)
		{
			if($i!=0)$result=strrev($a[$i]).".".$result;
			else $result=strrev($a[$i]).$result;
		}
		return $result;
	}    
    ...
    function vndate() {
        $date='';
        $date.=str_replace(array('Mon','Tue','Wed','Thu','Fri','Sat','Sun'),array('Thứ 2','Thứ 3','Thứ 4','Thứ 5','Thứ 6','Thứ 7','Chủ nhật'),date('D',NOW));
        $date.=', ngày '.date('d',gmmktime());
        $date.='/'.date('m',gmmktime());
        $date.='/'.date('Y',gmmktime());
        return $date;
    }
    ...
    function endate() {
        $date='';
        //$date.=str_replace(array('Mon','Tue','Wed','Thu','Fri','Sat','Sun'),array('Mon','Tue','Wed','Thu','Fri','Sat','Sun'),date('D',gmmktime()));
        //$date.=', day '.date('j',NOW);
        //$date.=' Month '.date('n',NOW);
        //$date.=' Year '.date('Y',NOW);
        $date=date('D, F d Y',gmmktime());
        //$date.=date('Y',gmmktime()).' / '.date('n',gmmktime()).' / '.date('j',gmmktime());
        return $date;
    }
	function splitText($str,$char)
    {
        if(strrpos($str,' ')>$char)
        {
            $newstr=substr($str,0,$char);
            $str=substr($newstr,0,strrpos($newstr,' '));
        } 
        return $str;
    }
	function change_name_file($file,$name) {
		$name_file = $this->title_folder($name);
    	$t_url = strtolower($file);
    	$ext = explode('.',$t_url);
    	$ext = $ext[count($ext)-1];
    	$ext = explode('?',$ext);
    ...
    {
        return "<textarea id='$name' name='$name' class='ckeditor'>$value</textarea><script language=\"JavaScript\">CKEDITOR.replace('".$name."',
				{
